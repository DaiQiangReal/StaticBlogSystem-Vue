<template>
  <div id="topbar">
      <div id="classification" v-for="classification of classifications" :key="JSON.stringify(classification)" :class="classification===highLight?'hight-light':''">
          {{Object.values(classification)[0]}}
      </div>
  </div>
</template>
    ...
    methods:{
        async getClassifications(){
            if(this.classifications.length!==0)
                return;
            let classTxtContentList=(await (await fetch("./blogs/name.txt")).text()).split("\n");
